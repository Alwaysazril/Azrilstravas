...
class UserController {
  ...
  static async getDashboardStats(req, res) {
    const { key } = req.query;
    if (!key) return res.json({ valid: false, message: 'Key diperlukan' });
    ...
    if (!user) return res.json({ valid: false, message: 'User tidak ditemukan' });

    // Hitung sisa hari
    let sisaHari = '∞';
    if (user.expiredDate && user.expiredDate !== 'lifetime') {
      try {
        const exp = new Date(user.expiredDate);
        const now = new Date();
        const diff = Math.ceil((exp - now) / (1000 * 60 * 60 * 24));
        sisaHari = diff > 0 ? diff.toString() : '0';
      } catch (_) { sisaHari = '?'; }
    }

    // Hitung online users (semua user yang login dalam 5 menit terakhir)
    const fiveMinsAgo = Date.now() - 5 * 60 * 1000;
    const onlineCount = Object.values(activeKeys)
      .filter(k => k.created && k.created > fiveMinsAgo).length;

    // Hitung sender count dari whatsapp sessions
    let senderCount = 0;
    try {
      const { sessions } = require('../services/whatsappService');
      senderCount = Object.keys(sessions || {}).filter(k => {
        const s = sessions[k];
        return s && s.sock && s.sock.user;
      }).length;
    } catch (_) {}
    ...
    return res.json({
      ...
      sisaHari,
      sender: senderCount,
      expiredDate: user.expiredDate || 'lifetime',
